...
process <- function(
  ...
  result <- if (file.exists(state_path)) {
    saved_result <- readRDS(state_path)
    if (!identical(as.list(prompts), saved_result@prompts)) {
  ...
  result <- if (is.null(result)) {
    ...
    new_result <- batch(
      ...
      responses = purrr::map(seq_along(prompts), ~NULL),
      completed = 0L,
  ...
  tryCatch({
    remaining_prompts <- (result@completed + 1L):total_prompts
    purrr::walk(remaining_prompts, function(i) {
...
process_parallel <- function(
  ...
  result <- if (file.exists(state_path)) {
    saved_result <- readRDS(state_path)
    if (!identical(prompts_list, saved_result@prompts)) {
      cli::cli_alert_warning("Prompts don't match saved state. Starting fresh.")
      unlink(state_path)
      NULL
    } else saved_result
  ...
  tryCatch({
    processed_chunks <- purrr::imap(chunks, function(chunk, chunk_idx) {
      attempt_result <- purrr::reduce(
        seq_len(max_chunk_attempts),
        function(acc, attempt) {
          ...
          chunk_result <- tryCatch({
            responses <- furrr::future_map(
              chunk,
              \(prompt) capture_with_retry(
                worker_chat, prompt, type_spec,
                echo = "none", timeout = timeout,
                max_retries = max_retries,
                initial_delay = initial_delay,
                max_delay = max_delay,
                backoff_factor = backoff_factor
              ),
              .options = furrr::furrr_options(scheduling = 1, seed = TRUE)
            )
            
            list(success = TRUE, responses = responses, tokens = worker_chat$tokens())
          },
          error = function(e) {
            structure(
              list(
          ...
          if (validation$valid) {
            ...
            list(done = TRUE, result = chunk_result)
          } else {
            if (attempt < max_chunk_attempts) {
              cli::cli_alert_warning(sprintf(
                "Chunk %d failed (attempt %d/%d): %s. Retrying...",
                chunk_idx, attempt, max_chunk_attempts, validation$message
              ))
              Sys.sleep(2^attempt)
            }
            list(done = FALSE, result = chunk_result)
          }
        },
        .init = list(done = FALSE, result = NULL)
      )
      
      if (!attempt_result$done) {
        stop(sprintf(
          "Chunk %d failed after %d attempts: %s",
          chunk_idx, max_chunk_attempts,
          validate_chunk_result(attempt_result$result, chunk_idx)$message
        ))
      }
...
process_chunks <- function(chunks, result, chat_obj, type_spec, pb, state_path, echo, beep) {
  ...
  purrr::walk(chunks, function(chunk) {
    if (was_interrupted) return(NULL)
