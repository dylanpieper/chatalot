...
#' @importFrom cli cli_progress_bar cli_progress_done cli_progress_clear
...
#' @name texts.batch
#' @param x A batch object
#' @return Character vector of response texts
#' @importFrom purrr map_chr
S7::method(texts, batch) <- function(x) {
  responses <- x@responses[seq_len(x@completed)]
  map_chr(responses, "text")
}
...
process <- function(chat_obj, prompts, type_spec = NULL,
  ...
  if (file.exists(state_path)) {
    ...
    if (!identical(prompts, result@prompts)) {
      unlink(state_path)
      result <- NULL
    }
  } else {
