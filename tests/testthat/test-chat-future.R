...
test_that("future_chat processes chunks correctly", {
  skip_chat_test()
  chat <- create_test_chat(future_chat)
  result <- chat$process(get_test_prompts(2), workers = 1, beep = FALSE)
...
test_that("future_chat processes content chunks correctly", {
  skip_chat_test()
  chat <- create_test_chat(future_chat)
  result <- chat$process(get_content_prompts(), workers = 1, beep = FALSE)
  validate_process_result(result, 2)
})
...
test_that("future_chat handles structured data extraction", {
  skip_chat_test()
  chat <- create_test_chat(future_chat)
  result <- chat$process(get_sentiment_prompts(), type = get_sentiment_type_spec(), workers = 1, beep = FALSE)
  data <- result$texts()
...
test_that("future_chat handles worker failures", {
  skip_chat_test()
  test_api_key_failure(future_chat, list(workers = 1))
})

test_that("future_chat supports progress parameter", {
  skip_chat_test()
  test_progress_parameter(future_chat, list(workers = 1))
})

test_that("future_chat doesn't break with echo parameter", {
  skip_chat_test()
  test_echo_parameter(future_chat, list(workers = 1))
})
