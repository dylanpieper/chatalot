...
test_that("multiple interruptions are handled correctly", {
  ...
  chat1 <- create_interruptible_chat(chat_batch, after_n_calls = 1, beep = FALSE)
  expect_error(
    chat1$batch(get_test_prompts(3), state_path = temp_file),
    class = "interrupt"
  )
  
  chat2 <- create_interruptible_chat(chat_batch, after_n_calls = 1, beep = FALSE)
  expect_error(
    chat2$batch(get_test_prompts(3), state_path = temp_file),
    class = "interrupt"
  )
  
  chat_final <- chat_batch(beep = FALSE)
